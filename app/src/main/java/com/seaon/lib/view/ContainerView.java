...
import com.seaon.lib.scale.TextStyleEntity;
...
import com.seaon.lib.util.AreaAveragingScale;
...
//        if (BuildConfig.DEBUG) {
//            canvas.drawColor(getResources().getColor(R.color.filter_color_red));
//        }
...
//            }
            if (BuildConfig.DEBUG) {
                Logger.d(TAG + "makeSize,bg==gif==>w;" + width + ",h:" + height);
            }
            if (width == 0 || height == 0) {
                if (BuildConfig.DEBUG) {
                    Logger.d(TAG + "GIF_W_H_ERROR");
                }
                return;
            }
            isDrawing = true;
    ...
    private void drawItem4GIf(Canvas canvas, float width, float height, float left, float top, float scale) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (view instanceof ScaleView) {
                ...
                matrix.postScale(scale, scale, 0, 0);
                if (BuildConfig.DEBUG) {
                    Logger.d("scale:" + scale + ",left:" + left + ",top:" + top);
                }
                canvas.save();
                canvas.concat(matrix);
                if (scaleView.getChildCount() > 0) {
                ...
                if (running == 1 && isMakingGifOrNot) {
                    ...
                    if (mGifMaker != null && !mGifMaker.isBitmapFull()) {
                        ...
                        if (System.currentTimeMillis() - startTime > mGifMaker.getTotalSize() * 1000) {
                            ...
                            if (BuildConfig.DEBUG) {
                                Logger.d(TAG + "bg==Gif>合成," + mGifMaker.getFrameCountNow() + ",time:" + time);
                            }
