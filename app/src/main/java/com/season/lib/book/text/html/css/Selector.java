...
//	}
	 ...
	 * @return
	 */
	public boolean isTheSelector(List<HtmlParser.TagInfo> tagPath) {
		boolean result = false;
		if(tagPath != null && !tagPath.isEmpty()) {
			HtmlParser.TagInfo tag = tagPath.get(tagPath.size() - 1);
			if(!TextUtils.isEmpty(name)) {
	...
	private boolean isTheCSS(String css, HtmlParser.TagInfo tag) {
		if(css.contains(".")) {
