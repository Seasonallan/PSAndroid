...
public class AccountID extends Hash160 {
    ...
    final public IKeyPair keyPair;

    public AccountID(IKeyPair kp, byte[] bytes) {
        super(bytes);
        //bytes数组是address的原bytes
        this.address = encodeAddress(bytes);
        this.keyPair = kp;
    }
    ...
    public static AccountID fromString(String value) {
        if (value.length() == 160 / 4) {
            ...
            if (Config.getB58().isXrp()){
                ...
                if (value.startsWith("a") && value.length() >= 26) {
                    return fromAddress(value);
                }
            }
    ...
    public String getPublicKey() {
        if (keyPair != null) {
            return keyPair.canonicalPubHex();
        } else {
            return null;
        }
    }
