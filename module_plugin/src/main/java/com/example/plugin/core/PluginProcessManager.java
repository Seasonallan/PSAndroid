...
import com.example.plugin.compat.ProcessCompat;
import com.example.lib.reflect.FieldUtils;
import com.example.lib.reflect.MethodUtils;
import com.example.plugin.stub.ActivityStub;
import com.example.plugin.stub.ServiceStub;
import com.example.plugin.hookcore.handle.PluginClassLoader;
import com.example.plugin.tool.PluginDirHelper;
                    ...
                    sHandle.post(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                LogUtil.e(TAG, "preMakeApplication FAIL", e);
                            } finally {
                                mExec.set(true);
                                synchronized (lock) {
                                    lock.notifyAll();
            ...
            for (ActivityInfo info : infos) {
                if (TextUtils.equals(info.processName, myPname)) {
                    try {
                        ...
                        LogUtil.i(TAG, "registerStaticReceiver error=%s", e, info.name);
                    }
                }
            }
        }
...
//        sSkipService.add("usagestats");
    ...
    private static void fakeSystemServiceInner(Context hostContext, Context targetContext) {
        try {
            ...
            if (SYSTEM_SERVICE_MAP != null && (SYSTEM_SERVICE_MAP instanceof Map)) {
                ...
                for (Object key : sSYSTEM_SERVICE_MAP.keySet()) {
                    ...
                    try {
                        ...
                        if (cause != null) {
                            LogUtil.w(TAG, "Fake system service faile", e);
                        } else {
                            LogUtil.w(TAG, "Fake system service faile", e);
                        }
                    } catch (Exception e) {
            ...
            LogUtil.e(TAG, "fakeSystemServiceOldAPI", e);
        }
    }
    ...
    public static void fakeSystemService(Context hostContext, Context targetContext) {
        if (VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH_MR1 && !TextUtils.equals(hostContext.getPackageName(), targetContext.getPackageName())) {
            ...
            LogUtil.i(TAG, "Fake SystemService for originContext=%s context=%s,cost %s ms", targetContext.getPackageName(), targetContext.getPackageName(), (System.currentTimeMillis() - b));
        }
    }
