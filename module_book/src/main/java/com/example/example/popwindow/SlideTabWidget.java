package com.example.example.popwindow;
...
public class SlideTabWidget extends TabWidget {
	...
	public void initialize(Drawable indicatorDrawable){
		initialize(0, LayoutParams.WRAP_CONTENT, indicatorDrawable);
	}
			...
			if(mDrawable.getIntrinsicWidth() > 0 && mDrawable.getIntrinsicHeight() > 0){
				if(mDrawHeight < 0){
					if(mDrawHeight == LayoutParams.WRAP_CONTENT){
						...
						mDrawWidth = mDrawable.getIntrinsicWidth() - mPaddingWidth * 2;
					}else{
						mDrawHeight = currentView.getHeight();
						mDrawWidth = currentView.getWidth() - mPaddingWidth * 2;
					}
				}else{
					mDrawWidth = mDrawable.getIntrinsicWidth() - mPaddingWidth * 2;
				}
				...
				if(mDrawHeight < 0){
					if(mDrawHeight == LayoutParams.WRAP_CONTENT){
						...
						mDrawHeight = currentView.getHeight();
						mDrawWidth = currentView.getWidth();
					}
				}else{
					mDrawWidth = minCotentWidth - mPaddingWidth * 2;
				}
